"""
Example showing how PyTorch can be called from Excel using PyXLL.
Requires PyTorch and pywin32 packages as well as PyXLL.

This example is an adaptation of an original article that can be found here:
...
@xl_func("object, float[]: float[]")
def nn_RunSingle(net, inputs):
    inputs = torch.tensor(inputs).type(torch.FloatTensor)
    result = net(inputs).detach().tolist()
    return result
